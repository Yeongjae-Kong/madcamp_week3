...
function App() {
  ...
  const detect = async () => {
    ...
    if (webcamRef.current && webcamRef.current.video.readyState === 4) {
      ...
      drawPose(poses);
      drawConnections(keypoints, EDGES, confidenceThreshold);
    }
  };
  ...
  // 연결을 그리는 함수
  const drawConnections = (keypoints, edges, confidenceThreshold, ctx, scaleX, scaleY) => {
...
const getColorForEdge = (colorCode) => {
  switch(colorCode) {
      case 'm':
          return 'magenta';
      case 'c':
...
// 연결 정보를 정의합니다.
  ...
  useEffect(() => {
    if (detector) {
      console.log('Starting interval for detection');
